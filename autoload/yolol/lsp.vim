...
function! s:newlsp() abort
  " job is the job used to talk to the backing instance of yodk langserv.
  ...
  "   * handleResult takes a single argument, the result message received from yodk langserv
  "   * error takes a single argument, the error message received from yodk langserv
  "     The error method is optional.
  " workspaceDirectories is an array of named workspaces.
  " wd is the working directory for yodk langserv 
  ...
  if !yolol#config#GoplsEnabled()
    let l:lsp.sendMessage = funcref('s:noop')
    return l:lsp
  endif

  if !yolol#util#has_job()
    let l:oldshortmess=&shortmess
    if has('nvim')
      set shortmess-=F
    endif
    call yolol#util#EchoWarning('Features that rely on yodk langserv will not work without either Vim 8.0.0087 or newer with +job')
  ...
  function! l:lsp.readMessage(data) dict abort
    ...
    while 1
      ...
      if empty(l:length_match)
        " TODO(bc): shutdown yodk langserv?
  ...
  function! l:lsp.handleRequest(req) dict abort
    if a:req.method == 'workspace/workspaceFolders'
      let l:resp = yolol#lsp#message#WorkspaceFoldersResult(self.workspaceDirectories)
    elseif a:req.method == 'workspace/configuration' && has_key(a:req, 'params') && has_key(a:req.params, 'items')
      let l:resp = yolol#lsp#message#ConfigurationResult(a:req.params.items)
    ...
    elseif a:req.method == 'workspace/applyEdit'
      try
        ...
        call yolol#util#EchoError(printf('could not apply edit: %s', v:exception))
        let l:ok = v:false
      endtry
      let l:resp = yolol#lsp#message#ApplyWorkspaceEditResponse(l:ok)
    else
      return
    endif
      ...
      try
        ...
        if has_key(a:resp, 'error')
          ...
          if has_key(l:handler, 'error')
            ...
            call yolol#util#EchoError(a:resp.error.message)
          endif
  ...
  function! l:lsp.updateDiagnostics() dict abort
    let l:level = yolol#config#DiagnosticsLevel()
    ...
    for l:data in self.diagnosticsQueue
      ...
      try
        ...
        let l:fname = yolol#path#FromURI(l:data.uri)
        ...
        call yolol#util#EchoError(printf('%s: %s', v:throwpoint, v:exception))
      endtry
    endfor
  endfunction

  function! l:lsp.handleInitializeResult(result) dict abort
    if yolol#config#EchoCommandInfo()
      call yolol#util#EchoProgress("initialized yodk langserv")
    endif
    let status = {
          \ 'desc': '',
          \ 'type': 'yodk langserv',
          \ 'state': 'initialized',
        \ }
    call yolol#statusline#Update(self.wd, status)

    let self.ready = 1
    let  l:msg = self.newMessage(yolol#lsp#message#Initialized())
    call self.write(l:msg)
  ...
  function! l:lsp.sendMessage(data, handler) dict abort
    if !self.last_request_id
      let l:wd = yolol#util#ModuleRoot()
      if l:wd == -1
        call yolol#util#EchoError('could not determine appropriate working directory for yodk langserv')
        return -1
      endif
      ...
      if yolol#config#EchoCommandInfo()
        call yolol#util#EchoProgress("initializing yodk langserv")
      endif
      ...
      let l:status = {
            ...
            \ 'state': 'initializing',
          \ }
      call yolol#statusline#Update(l:wd, l:status)

      let self.workspaceDirectories = add(self.workspaceDirectories, l:wd)
      let l:msg = self.newMessage(yolol#lsp#message#Initialize(l:wd))
  ...
  function! l:lsp.write(msg) dict abort
    ...
    try
      ...
      call yolol#util#EchoError(printf('could not send message: %s', v:exception))
    endtry
  endfunction
  ...
  function! l:lsp.exit_cb(job, exit_status) dict
    ...
    " restore workspaces
    call call('yolol#lsp#AddWorkspaceDirectory', l:workspaces)
    " * send DidOpen messages for all buffers that have b:did_lsp_open set
    " TODO(bc): check modifiable and filetype, too?
    bufdo if get(b:, 'yolol_lsp_did_open', 0) | if &modified | call yolol#lsp#DidOpen(expand('%:p')) | else | call yolol#lsp#DidChange(expand('%:p')) | endif | endif
    let l:lsp.queue = extend(l:lsp.queue, l:queue)
    return
  endfunction

  function! l:lsp.close_cb(ch) dict abort
    " TODO(bc): remove the buffer variables that indicate that yodk langservhas been
  ...
  let l:bin_path = yolol#path#CheckBinPath("yodk")
  if empty(l:bin_path)
    return l:lsp
  endif

  let l:cmd = [l:bin_path]
  let l:cmdopts = yolol#config#GoplsOptions()

  if yolol#util#HasDebug('lsp')
    " debugging can be enabled either with g:yolol_debug
    let l:needsDebug = 1
  ...
  let l:lsp.job = yolol#job#Start(l:cmd+l:cmdopts, l:opts)
...
function! s:requestComplete(ok) abort dict
  ...
  if yolol#config#EchoCommandInfo()
    ...
    if a:ok
      call yolol#util#EchoSuccess(prefix . "SUCCESS")
    else
      call yolol#util#EchoError(prefix . "FAIL")
    endif
  endif
  ...
  call yolol#statusline#Update(self.jobdir, status)
endfunction
...
" yolol#lsp#Definition calls yodk langserv to get the definition of the identifier at
...
" etc.)
function! yolol#lsp#Definition(fname, line, col, handler) abort
  call yolol#lsp#DidChange(a:fname)
  ...
  let l:msg = yolol#lsp#message#Definition(fnamemodify(a:fname, ':p'), a:line, a:col)
...
function! s:definitionHandler(next, msg) abort dict
  ...
  " yodk langserv returns a []Location; just take the first one.
  let l:msg = a:msg[0]
  let l:args = [[printf('%s:%d:%d: %s', yolol#path#FromURI(l:msg.uri), l:msg.range.start.line+1, yolol#lsp#lsp#PositionOf(getline(l:msg.range.start.line+1), l:msg.range.start.character), 'lsp does not supply a description')]]
  call call(a:next, l:args)
endfunction

" yolol#lsp#Type calls yodk langserv to get the type definition of the identifier at
" line and col in fname. handler should be a dictionary function that takes a
" list of strings in the form 'file:line:col: message'. handler will be
" attached to a dictionary that manages state (statuslines, sets the winid,
" etc.)
function! yolol#lsp#TypeDef(fname, line, col, handler) abort
  call yolol#lsp#DidChange(a:fname)

  let l:lsp = s:lspfactory.get()
  let l:state = s:newHandlerState('type definition')
  let l:msg = yolol#lsp#message#TypeDefinition(fnamemodify(a:fname, ':p'), a:line, a:col)
...
" yolol#lsp#Callers calls yodk langserv to get callers of the identifier at
" line and col in fname. handler should be a dictionary function that takes a
" list of strings in the form 'file:line:col: message'. handler will be
" attached to a dictionary that manages state (statuslines, sets the winid,
" etc.)
function! yolol#lsp#Callers(fname, line, col, handler) abort
  call yolol#lsp#DidChange(a:fname)

  let l:lsp = s:lspfactory.get()
  let l:state = s:newHandlerState('callers')
  let l:msg = yolol#lsp#message#PrepareCallHierarchy(fnamemodify(a:fname, ':p'), a:line, a:col)
...
function! s:prepareCallHierarchyHandler(next, msg) abort dict
  ...
  let l:lsp = s:lspfactory.get()
  let l:state = s:newHandlerState('callers')
  let l:msg = yolol#lsp#message#IncomingCalls(a:msg[0])
...
function! s:incomingCallsHandler(next, msg) abort dict
  ...
  for l:item in a:msg
    try
      let l:fname = yolol#path#FromURI(l:item.from.uri)
      ...
      for l:fromRange in l:item.fromRanges
        ...
        let l:locations = add(l:locations, printf('%s:%s:%s: %s', l:fname, l:line, yolol#lsp#lsp#PositionOf(content, l:fromRange.start.character), l:item.from.name))
      endfor
    catch
    endtry
  endfor
...
function! yolol#lsp#DidOpen(fname) abort
  if get(b:, 'yolol_lsp_did_open', 0)
    return
  endif
  ...
  let l:msg = yolol#lsp#message#DidOpen(l:fname, join(yolol#util#GetLines(), "\n") . "\n", l:lsp.fileVersions[l:fname])
  let l:state = s:newHandlerState('')
  ...
  let b:yolol_lsp_did_open = 1

  return l:lsp.sendMessage(l:msg, l:state)
endfunction

function! yolol#lsp#DidChange(fname) abort
  " DidChange is called even when fname isn't open in a buffer (e.g. via
  " yolol#lsp#Info); don't report the file as open or as having changed when it's
  ...
  call yolol#lsp#DidOpen(a:fname)
  ...
  let l:msg = yolol#lsp#message#DidChange(l:fname, join(yolol#util#GetLines(), "\n") . "\n", l:lsp.fileVersions[l:fname])
  let l:state = s:newHandlerState('')
  return l:lsp.sendMessage(l:msg, l:state)
endfunction

function! yolol#lsp#DidClose(fname) abort
  let l:fname = fnamemodify(a:fname, ':p')
  if !isdirectory(fnamemodify(l:fname, ':h'))
    return
  endif
  ...
  let l:lsp = s:lspfactory.get()
  let l:msg = yolol#lsp#message#DidClose(l:fname)
  let l:state = s:newHandlerState('')
  " TODO(bc): setting a buffer level variable here assumes that a:fname is the
  " current buffer. Change to a:fname first before setting it and then change
  " back to active buffer.
  let b:yolol_lsp_did_open = 0

  return l:lsp.sendMessage(l:msg, l:state)
endfunction

function! yolol#lsp#Completion(fname, line, col, handler) abort
  call yolol#lsp#DidChange(a:fname)

  let l:lsp = s:lspfactory.get()
  let l:msg = yolol#lsp#message#Completion(a:fname, a:line, a:col)
...
function! s:completionHandler(next, msg) abort dict
  " yodk langserv returns a CompletionList.
  let l:matches = []
  let l:start = -1
  ...
  for l:item in a:msg.items
    ...
    let l:match = {'abbr': l:item.label, 'word': l:item.textEdit.newText, 'info': '', 'kind': yolol#lsp#completionitemkind#Vim(l:item.kind), 'user_data': '', 'icase': yolol#config#CodeCompletionIcase()}
    if has_key(l:item, 'detail')
        let l:match.menu = l:item.detail
        if yolol#lsp#completionitemkind#IsFunction(l:item.kind) || yolol#lsp#completionitemkind#IsMethod(l:item.kind)
          let l:match.info = printf('%s %s', l:item.label, l:item.detail)

          " The detail provided by yodk langserv hasn't always provided the the full
...
" yolol#lsp#SameIDs calls yodk langserv to get the references to the identifier at line
...
function! yolol#lsp#SameIDs(showstatus, fname, line, col, handler) abort
  call yolol#lsp#DidChange(a:fname)

  let l:lsp = s:lspfactory.get()
  let l:msg = yolol#lsp#message#References(a:fname, a:line, a:col)
...
function! s:sameIDsHandler(next, msg) abort dict
  let l:furi = yolol#path#ToURI(expand('%:p'))
  ...
  for l:loc in l:msg
    ...
    let l:result.sameids = add(l:result.sameids, printf('%s:%s:%s', yolol#path#FromURI(l:loc.uri), l:loc.range.start.line+1, l:loc.range.start.character+1))
  endfor

  call call(a:next, [0, json_encode(l:result), ''])
endfunction

" yolol#lsp#Referrers calls yodk langserv to get the references to the identifier at line
" and col in fname. handler should be a dictionary function that takes a list
" of strings in the form 'file:line:col: message'. handler will be attached to
" a dictionary that manages state (statuslines, sets the winid, etc.). handler
" should take three arguments: an exit_code, a JSON object encoded to a string
...
" exists for compatibility with the guru implementation of SameIDs.
" TODO(bc): refactor to not need the guru adapter.
function! yolol#lsp#Referrers(fname, line, col, handler) abort
  call yolol#lsp#DidChange(a:fname)
...
function! s:handleLocations(next, msg) abort
  ...
  for l:loc in l:msg
    let l:fname = yolol#path#FromURI(l:loc.uri)
    ...
    let l:item = printf('%s:%s:%s: %s', l:fname, l:line, yolol#lsp#lsp#PositionOf(l:content, l:loc.range.start.character), l:content)
...
" yolol#lsp#Implementations calls yodk langserv to get the implementations to the
...
function! yolol#lsp#Implements(fname, line, col, handler) abort
  call yolol#lsp#DidChange(a:fname)

  let l:lsp = s:lspfactory.get()
  let l:msg = yolol#lsp#message#Implementation(a:fname, a:line, a:col)
...
function! yolol#lsp#Hover(fname, line, col, handler) abort
  call yolol#lsp#DidChange(a:fname)

  let l:lsp = s:lspfactory.get()
  let l:msg = yolol#lsp#message#Hover(a:fname, a:line, a:col)
  let l:state = s:newHandlerState('')
...
function! yolol#lsp#Doc() abort
  let l:fname = expand('%:p')
  let [l:line, l:col] = yolol#lsp#lsp#Position()

  call yolol#lsp#DidChange(l:fname)

  let l:lsp = s:lspfactory.get()
  let l:msg = yolol#lsp#message#Hover(l:fname, l:line, l:col)
  let l:state = s:newHandlerState('doc')
  let l:resultHandler = yolol#promise#New(function('s:docFromHoverResult', [], l:state), 10000, '')
...
function! yolol#lsp#DocLink() abort
  let l:fname = expand('%:p')
  let [l:line, l:col] = yolol#lsp#lsp#Position()
  ...
  let l:resultHandler = yolol#promise#New(function('s:docLinkFromHoverResult', [], l:state), 10000, '')
  let l:state.handleResult = l:resultHandler.wrapper
  let l:state.error = l:resultHandler.wrapper
  call l:lsp.sendMessage(l:msg, l:state)
  return l:resultHandler.await()
...
function! s:docLinkFromHoverResult(msg) abort dict
  ...
  " for backward compatibility with older yodk
...
function! yolol#lsp#Info(showstatus)
  let l:fname = expand('%:p')
  let [l:line, l:col] = yolol#lsp#lsp#Position()
  ...
  let l:state.error = funcref('s:noop')
  let l:msg = yolol#lsp#message#Definition(l:fname, l:line, l:col)
  return l:lsp.sendMessage(l:msg, l:state)
endfunction

function! yolol#lsp#GetInfo()
  let l:fname = expand('%:p')
  let [l:line, l:col] = yolol#lsp#lsp#Position()
  ...
  let l:info = yolol#promise#New(function('s:info', [0], l:state), 10000, '')
...
function! s:infoDefinitionHandler(next, showstatus, msg) abort dict
  ...
  let l:fname = yolol#path#FromURI(l:msg.uri)
...
function! s:info(show, msg) abort dict
  ...
  if a:show
    call yolol#util#ShowInfo(l:content)
  endif
...
function! yolol#lsp#AddWorkspaceDirectory(...) abort
  if a:0 == 0
    return
  endif

  call yolol#lsp#CleanWorkspaces()
  ...
  let l:msg = yolol#lsp#message#ChangeWorkspaceFolders(l:workspaces, [])
  call l:lsp.sendMessage(l:msg, l:state)

  return 0
endfunction

function! yolol#lsp#CleanWorkspaces() abort
  let l:workspaces = []
  ...
  let l:state = s:newHandlerState('')
  let l:msg = yolol#lsp#message#ChangeWorkspaceFolders([], s:dedup(l:missing))
  call l:lsp.sendMessage(l:msg, l:state)

  return 0
endfunction

" yolol#lsp#ResetWorkspaceDiretories removes and then re-adds all workspace
" folders to cause yodk langserv to send configuration requests for all of them again.
" This is useful, for instance, when build tags have been added and yodk langserv
" needs to use them.
function! yolol#lsp#ResetWorkspaceDirectories() abort
  call yolol#lsp#CleanWorkspaces()

  let l:lsp = s:lspfactory.get()

  let l:state = s:newHandlerState('')
  let l:msg = yolol#lsp#message#ChangeWorkspaceFolders(s:dedup(l:lsp.workspaceDirectories), s:dedup(l:lsp.workspaceDirectories))
  call l:lsp.sendMessage(l:msg, l:state)

  return 0
endfunction

function! yolol#lsp#DebugBrowser() abort
  let l:lsp = s:lspfactory.get()
  let l:port = get(l:lsp, 'debugport', 0)
  if !l:port
    call yolol#util#EchoError("yodk langserv was not started with debugging enabled. See :help g:yolol_debug.")
    return
  endif

  call yolol#util#OpenBrowser(printf('http://localhost:%d', l:port))
endfunction

function! yolol#lsp#Exit() abort
  call s:exit(0)
endfunction

function! yolol#lsp#Restart() abort
  call s:exit(1)
endfunction

function! s:exit(restart) abort
  if !yolol#util#has_job() || len(s:lspfactory) == 0 || !has_key(s:lspfactory, 'current')
    return
  endif
  ...
  let l:msg = yolol#lsp#message#Shutdown()
  let l:retval = l:lsp.sendMessage(l:msg, l:state)

  let l:msg = yolol#lsp#message#Exit()
  let l:retval = l:lsp.sendMessage(l:msg, l:state)
...
function! yolol#lsp#Diagnostics(...) abort
  if a:0 == 0
    return []
  endif
  ...
  for [l:key, l:val] in items(l:lsp.diagnostics)
    ...
    if !has_key(l:dirsToPackages, l:dir)
      let l:pkg = yolol#package#FromPath(l:dir)
...
function! yolol#lsp#AnalyzeFile(fname) abort
  let l:fname = fnamemodify(a:fname, ':p')
  if !isdirectory(fnamemodify(l:fname, ':h'))
    return []
  endif
  ...
  let l:diagnostics = yolol#promise#New(function('s:setDiagnostics', []), 10000, l:lastdiagnostics)
...
function! s:errorFromDiagnostic(diagnostic, bufname, fname) abort
  ...
  let l:col = ''
  if len(l:buflines) > 0
    let l:col = yolol#lsp#lsp#PositionOf(l:buflines[0], l:range.start.character)
  endif
  let l:error = printf('%s:%s:%s:%s: %s', a:fname, l:line, l:col, yolol#lsp#lsp#SeverityToErrorType(a:diagnostic.severity), a:diagnostic.message)
  ...
  let l:endcol = yolol#lsp#lsp#PositionOf(getline(l:endline), l:range.end.character)
...
function! s:highlightMatches(errorMatches, warningMatches) abort
  ...
  let b:yolol_diagnostic_matches = {'errors': [], 'warnings': []}
  ...
  if hlexists('goDiagnosticError')
    ...
    " be properly cleared.
    call yolol#util#ClearHighlights('goDiagnosticError')
    if yolol#config#DiagnosticsLevel() >= 2
      let b:yolol_diagnostic_matches.errors = copy(a:errorMatches)
      if yolol#config#HighlightDiagnosticErrors()
        call yolol#util#HighlightPositions('goDiagnosticError', a:errorMatches)
      endif
    endif
  endif
  ...
  if hlexists('goDiagnosticWarning')
    ...
    " changed the level since the last highlighting, the highlighting will be
    " be properly cleared.
    call yolol#util#ClearHighlights('goDiagnosticWarning')
    if yolol#config#DiagnosticsLevel() >= 2
      let b:yolol_diagnostic_matches.warnings = copy(a:warningMatches)
      if yolol#config#HighlightDiagnosticWarnings()
        call yolol#util#HighlightPositions('goDiagnosticWarning', a:warningMatches)
      endif
    endif
  endif
  ...
  augroup vim-go-diagnostics
    ...
    if has('textprop')
      autocmd BufReadPost <buffer> nested call s:highlightMatches(b:yolol_diagnostic_matches.errors, b:yolol_diagnostic_matches.warnings)
    else
      autocmd BufWinEnter <buffer> nested call s:highlightMatches(b:yolol_diagnostic_matches.errors, b:yolol_diagnostic_matches.warnings)
    endif
  augroup end
endfunction
...
function! yolol#lsp#ClearDiagnosticHighlights() abort
  call yolol#util#ClearHighlights('goDiagnosticError')
  call yolol#util#ClearHighlights('goDiagnosticWarning')
endfunction

" Format formats the current buffer.
function! yolol#lsp#Format() abort
  let l:fname = expand('%:p')
  ...
  let l:state = s:newHandlerState('')
  let l:handleFormat = yolol#promise#New(function('s:handleFormat', [], l:state), 10000, '')
  ...
  let l:msg = yolol#lsp#message#Format(l:fname)
  call l:lsp.sendMessage(l:msg, l:state)
...
" buffer.
function! yolol#lsp#Imports() abort
  let l:fname = expand('%:p')
  " send the current file so that TextEdits will be relative to the current
  " state of the buffer.
  call yolol#lsp#DidChange(l:fname)
  ...
  let l:state = s:newHandlerState('')
  let l:handler = yolol#promise#New(function('s:handleCodeAction', ['source.organizeImports', ''], l:state), 10000, '')
  ...
  let l:msg = yolol#lsp#message#CodeActionImports(l:fname)
  call l:lsp.sendMessage(l:msg, l:state)
...
" current location.
function! yolol#lsp#FillStruct() abort
  let l:fname = expand('%:p')
  " send the current file so that TextEdits will be relative to the current
  " state of the buffer.
  call yolol#lsp#DidChange(l:fname)
  ...
  let l:state = s:newHandlerState('')
  let l:handler = yolol#promise#New(function('s:handleCodeAction', ['refactor.rewrite', 'fill_struct'], l:state), 10000, '')
  let l:state.handleResult = l:handler.wrapper
  let l:state.error = l:handler.wrapper
  let l:state.handleError = function('s:handleCodeActionError', [l:fname], l:state)

  let [l:line, l:col] = yolol#lsp#lsp#Position()
  let l:msg = yolol#lsp#message#CodeActionFillStruct(l:fname, l:line, l:col)
  call l:lsp.sendMessage(l:msg, l:state)
...
function! s:executeCommand(cmd, args) abort
  ...
  let l:msg = yolol#lsp#message#ExecuteCommand(a:cmd, a:args)
  call l:lsp.sendMessage(l:msg, l:state)
endfunction
  ...
  for l:item in a:msg
    if get(l:item, 'kind', '') is a:kind
      ...
      if has_key(l:item, 'command')
        if has_key(l:item.command, 'command') && (l:item.command.command is a:cmd || l:item.command.command is printf('yodk.%s', a:cmd))
...
function s:applyTextEdits(msg) abort
  ...
  for l:msg in a:msg
    ...
    if l:msg.range.start.character > 0
      let l:preSliceEnd = yolol#lsp#lsp#PositionOf(l:startcontent, l:msg.range.start.character-1) - 1
    ...
    if l:msg.range.end.character > 0
      let l:postSliceStart = yolol#lsp#lsp#PositionOf(l:endcontent, l:msg.range.end.character-1)
  ...
  call yolol#lsp#DidChange(expand('%:p'))
  return
endfunction

function! s:handleFormatError(filename, msg) abort dict
  if yolol#config#FmtFailSilently()
    return
  endif
  ...
  call yolol#fmt#ShowErrors(l:errors)
endfunction
...
function! s:ensureWorkspace(dir)
  let l:modroot = yolol#util#ModuleRoot(a:dir)
  ...
  call yolol#lsp#AddWorkspaceDirectory(l:modroot)
endfunction
...
function! s:lineinfile(fname, line) abort
  ...
  try
    ...
    call yolol#util#EchoError(printf('%s (line %s): %s at %s', a:fname, a:line, v:exception, v:throwpoint))
    return -1
  endtry
endfunction
